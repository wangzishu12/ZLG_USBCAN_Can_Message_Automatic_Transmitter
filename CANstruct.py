...
class VCI_CAN_OBJ(Structure):
    ...
    def __str__(self):
        ...
        TimeStampTemp = str(int(self.TimeStamp))[:-4] + "." + str(int(self.TimeStamp))[-4:]
        return 'ID:%08X,时间戳:%s,长度:%X,数据:%s'%(self.ID, TimeStampTemp, self.DataLen, datastr)

    def getdata(self):
        return [format(self.Data[i],"02X") for i in range(self.DataLen)]
